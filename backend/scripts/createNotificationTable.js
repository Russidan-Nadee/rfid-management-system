...
async function createNotificationTable() {
  ...
  try {
    ...
    if (existing[0].count > 0) {
      console.log('✅ problem_notification table already exists');
      return;
    }
    
    console.log('Creating problem_notification table...');
    ...
    console.log('Table created, now adding foreign key constraints...');
    
    // Add foreign key constraints separately
    await connection.execute(`
      ALTER TABLE \`problem_notification\`
      ADD CONSTRAINT \`fk_problem_notification_asset\` 
    ...
    await connection.execute(`
      ALTER TABLE \`problem_notification\`
      ADD CONSTRAINT \`fk_problem_notification_reporter\` 
    ...
    await connection.execute(`
      ALTER TABLE \`problem_notification\`
      ADD CONSTRAINT \`fk_problem_notification_acknowledger\` 
    ...
    await connection.execute(`
      ALTER TABLE \`problem_notification\`
      ADD CONSTRAINT \`fk_problem_notification_resolver\` 
        FOREIGN KEY (\`resolved_by\`) REFERENCES \`mst_user\` (\`user_id\`) 
        ON DELETE SET NULL ON UPDATE CASCADE
    `);
    
    console.log('✅ problem_notification table created successfully');
